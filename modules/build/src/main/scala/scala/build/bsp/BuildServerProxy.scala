...
class BuildServerProxy(
  ...
  override def buildTargetFormat(params: FormatParams): CompletableFuture[AnyRef] = {
    ...
    val formatItem = params.getFormatItems.get(0)
    ...
    val diagnostics = new b.PublishDiagnosticsParams(
      ...
      formatItem.getTarget,
      List(new b.Diagnostic(dummyRange, formatItem.getUris.asScala.mkString("[", ",", "]"))).asJava,
      false
    )
